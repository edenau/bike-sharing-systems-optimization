...
for Tslice = 1:72
    ...
    fl(fl<1e-6) = 0; % under-empty stations will be set as empty
    ...
    if min(emptylevel) < -1e-6 % If so, error message
        fprintf('Station %d with empty level %.2f at Tslice %d\n', ...
            find(emptylevel == min(emptylevel),1), min(emptylevel), Tslice);
    end
    
    emptylevel(emptylevel<0) = 0; % avoid unbounded obj function
    ...
    if ~skipflag % if there are some Arrivals
        while k<k_max && ~converged
            ...
            converged = true(1);
            for checki = 1:ProblemSize
                if (sum(xhat(:,checki,k+1))) - emptylevel(checki) > 1e-6
                    %|| sum(round(xhat(checki,:,k+1))) ~= Arrival(checki,Tslice) % or if any row (after round off) gets incorrect #Arrival
